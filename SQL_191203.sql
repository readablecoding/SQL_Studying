...
---------------일반함수---------------
SELECT NUM, NAME, NVL(EXAM + 1, 0)EXAM FROM PERSON; --EXAM + 1이 NULL이면 0으로 
SELECT NUM, NAME, NVL2(EXAM + 1, 0, 1)EXAM FROM PERSON; --EXAM + 1이 NULL이 아니면 0, NULL이면 1
SELECT NUM, NAME, NVL(PHONE, '0000') PHONE FROM PERSON; --전화번호가 NULL이면 '0000'으로 나타내기

--전화번호가 있으면 맨 앞 3자리만, 없으면 "000"으로
-->합격자 중에서 고른다. NVL2로 처리, NULL이면 000, 있으면 3글자만 잘라옴
SELECT 
    NAME 
...
--PASS 컬럼에 'O'가 들어있으면 '합격', 'X'가 들어있으면 '불합격',그 외는 '미정'으로 처리
--1)DECODE 사용
SELECT 
    NAME
    ,DECODE(PASS,'O', '합격'      --PASS 칼럼의 값이 'O'이면 '합격'
                ,'X', '불합격'     --PASS 칼럼의 갑싱 'X'이면 '불합격'
                ,'미정') AS PASS    --이것도 저것도 아니면 '미정'            
FROM 
    PERSON; 
...
SELECT
    ...
    ,(CASE 
        ...
        ELSE '미정'  
    END) AS PASS --CASE~END까지가 하나의 칼럼이다.끝나고 별칭 붙이기
FROM 
    PERSON;


--모든 지원자의 개수, 합계,평균, 최대값, 최소값은?
SELECT COUNT(*) FROM PERSON;    --13 -> COUNT함수는 괄호 속에 칼럼명 대신 *을 넣어도 된다.
SELECT SUM(EXAM) FROM PERSON;   --679
SELECT AVG(EXAM) FROM PERSON;   --52.23076923076923076923076923076923076923
SELECT MAX(EXAM) FROM PERSON;   --95
SELECT MIN(EXAM) FROM PERSON;   --10
...
SELECT 
    ...
    ,AVG(EXAM) 
FROM 
    PERSON; --"not a single-group group function" -> 합계, 평균운 1*1인데 이름은 행 길이가 길다.

--주소에 광주가 들어가는 사람 수 계산
SELECT 
    COUNT(*) 
FROM 
    PERSON 
