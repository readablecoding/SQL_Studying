...
--19/12/6 금
--<Join>
SELECT * FROM MEMBER; --5개의 행, 7개의 열
SELECT * FROM BOARD;    --10개의 행, 7개의 열
SELECT * FROM MEMBER,BOARD; --테이블 이름을 2개 이어쓰면 50개의 행, 14개의 열 -> 행의 수는 곱하고, 열의 수는 곱함
SELECT * FROM MEMBER, BOARD WHERE MEMBER.ID = BOARD.ID; --조인
SELECT * FROM MEMBER M , BOARD B WHERE M.ID = B.ID; --테이블 이름에 별칭을 붙여 조건을 쓸 때 편리함

--INNER JOIN -> 양쪽에 관련있는 게시판에 글 쓴 멤버 중 글번호, ID, NAME, TITLE
...
--OUTER JOIN은 모자라는 쪽에 (+)을 해준다. 
--RIGHT OUTER JOIN -> 오른쪽 테이블에 관계없는 데이터가 들어있는 것을 보여주기 위함
SELECT NUM, M.ID, M.NAME, B.TITLE FROM MEMBER M, BOARD B WHERE M.ID(+) = B.ID; --보드기준으로 없는 데이터 가져오기
SELECT NUM, M.ID, M.NAME, B.TITLE FROM MEMBER M RIGHT OUTER JOIN BOARD B ON M.ID = B.ID;

--LEFT OUTER JOIN -> 게시판에 글 안 쓴 멤버도 포함된 것이 나온다. NVL로 글 없음 처리 가능
SELECT NUM, M.ID, M.NAME, B.TITLE FROM MEMBER M, BOARD B WHERE M.ID = B.ID(+); --멤버기준으로 없는 데이터 가져오기
SELECT NUM, M.ID, M.NAME, B.TITLE FROM MEMBER M LEFT OUTER JOIN BOARD B ON M.ID = B.ID;

--FULL OUTER JOIN
--SELECT NUM, M.ID, M.NAME, B.TITLE FROM MEMBER M, BOARD B WHERE M.ID(+) = B.ID(+); --양쪽에 +를 붙일 수는 없다
...
--> 리플이 없는 글이 나오는 것이 왼쪽이므로 (+)를 오른쪽에 붙임
